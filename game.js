...
(function(){
  ...
  function applyEngineMove(uci) {
    ...
    try {
      ...
      saveGame(); updateCaptured(); updateHistoryUI(); recolorBoard(); checkGameEndAndUpdate();
    } catch(e) {
      console.error("Engine move failed:", e);
    } finally {
      botThinking = false;
    playerThinking = false; // ensure player can move again
    }
  }
...
function wireButtons() {
  ...
  undoBtn?.addEventListener("click", () => {
    // single undo (undo last ply). If you prefer undo both plies, call twice.
    try { game.undo(); } catch(e) {}
    board.position(game.fen());
    saveGame();
    updateCaptured();
